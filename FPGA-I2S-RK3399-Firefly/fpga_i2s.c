...
{
    volatile unsigned int nfifo,nfifo0;
    struct fpga_i2s_dev *i2s=filp->private_data; 
    int index=0;
    int ret=0;
    //read finished,reset flag.
    i2s->brx_okay=false;
    //re-enable irq.
    enable_irq(i2s->irq);
    ...
    if(0==wait_event_interruptible(i2s->wait_queue,i2s->brx_okay))
    {
    	...
    	nfifo0=(nfifo>>0)&0x3F;
    	ret=nfifo0*sizeof(char)*3;//24-bit.
    	//printk(KERN_INFO DRV_NAME ":rx fifo0 %d\n",nfifo);
    	if(count<nfifo0)
   	 {
		printk(KERN_INFO DRV_NAME ":read buffer is not enough!(%d/%d)\n",nfifo0,(int)count); 
		return -1;
    	}
    	//read data from RxFIFO.
    	while(nfifo0-->0)
    	{
        	volatile unsigned int pcm;
		...
		//0x12345600
		//bytes[0]=(unsigned char)(pcm&0xFF);//00
		//bytes[1]=(unsigned char)((pcm&0xFF00)>>8);//56
		//bytes[2]=(unsigned char)((pcm&0xFF0000)>>16);//34	
		//newbytes=(unsigned char)((pcm&0xFF000000)>>24);//12	
		//printk(KERN_INFO DRV_NAME "%x:%02x %02x %02x %02x\n",pcm,bytes[0],bytes[1],bytes[2],newbytes);
		//Big-endian.
		...
		if(0!=copy_to_user((void*)(buf+index),(void*)bytes,sizeof(bytes)))
		{
			printk(KERN_INFO DRV_NAME ":copy_to_user failed!\n");
			return -1;
		}
		index+=sizeof(char)*3;//24-bits	
	}
    }
    return ret;
}
...
////////////////////////////////////////////////////
#define GRF_GPIO3D_IOMUX 0xFF770000
static int fpga_i2s_probe(struct platform_device *pdev)
{
    ...
    unsigned long *pbase=ioremap(GRF_GPIO3D_IOMUX+0x0e01c,4);
    printk("val=%x\n",ioread32(pbase));
    iounmap(pbase);
    ...
    //2.bypass I2S_TXCR,we donot care tx part.
    ...
    val=val&(~I2S_RXCR_FBM_SHIFT);//MSB.
    ...
    //6.clear all rx&tx logic.
    val=ioread32(i2s->paddr_i2s_clr);
    val=val|I2S_CLR_RXC;
    val=val|I2S_CLR_TXC;
    printk(KERN_INFO DRV_NAME "I2S_CLR:%x\n",val);
...
//////////////////////////////////////////////////////////////
MODULE_DESCRIPTION("RK3399 FPGA-I2S Driver Module");
