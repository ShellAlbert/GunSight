...
//    }
    ...
    while(!gGblPara.m_bGblRst2Exit && !this->m_bExitFlag)
    {
        ...
        qint32 nYUV422Len=0;
        nYUV422Len=this->m_rbYUV->ZGetElement((qint8*)pYUV422Buffer,BUFSIZE_1MB);
        if(nYUV422Len<0)
        {
            qDebug()<<"<Error>:timeout to get yuv from yuv queue.";
            continue;
        }
        ...
        if(ret<0)
        {
            ...
            if(nEncodeBytes>0)
            {
                if(this->m_rbH264->ZPutElement((qint8*)pEncodeBuffer,nEncodeBytes)<0)
                {
                    qDebug()<<"<Warning>:timeout to put h264 to h264 queue.";
                }
            }
