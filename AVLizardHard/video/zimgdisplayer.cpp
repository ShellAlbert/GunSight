...
ZImgDisplayer::ZImgDisplayer(qint32 nCenterX,qint32 nCenterY,bool bMainCamera,QWidget *parent) : QWidget(parent)
{
    ...
    if(this->m_bMainCamera)
    {
            this->m_nShadowX=gGblPara.m_calibrateX1;
            this->m_nShadowY=gGblPara.m_calibrateY1;
    }else{
            this->m_nShadowX=gGblPara.m_calibrateX2;
            this->m_nShadowY=gGblPara.m_calibrateY2;
    }
}
...
void ZImgDisplayer::resizeEvent(QResizeEvent *event)
{
    ...
    //摄像头捕获的图像分辨率，这里固定写死。
    this->m_nCamWidth=gGblPara.m_widthCAM1;
    this->m_nCamHeight=gGblPara.m_heightCAM1;
...
void ZImgDisplayer::ZSlotFlushImg(const QImage &img)
{
    ...
    if(this->m_bMainCamera)
    {
        if(this->m_nShadowX!=gGblPara.m_calibrateX1 || this->m_nShadowY!=gGblPara.m_calibrateY1)
        {
            this->m_nShadowX=gGblPara.m_calibrateX1;
            this->m_nShadowY=gGblPara.m_calibrateY1;
            //cause to resizeEvent().
            this->resize(this->width(),this->height());
        }
    }else{
        if(this->m_nShadowX!=gGblPara.m_calibrateX2 || this->m_nShadowY!=gGblPara.m_calibrateY2)
        {
            //cause to resizeEvent().
            this->m_nShadowX=gGblPara.m_calibrateX2;
            this->m_nShadowY=gGblPara.m_calibrateY2;
