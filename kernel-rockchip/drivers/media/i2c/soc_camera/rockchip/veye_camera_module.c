/*
 * veye_camera_module.c
 *
 * Generic veye sensor driver
 *
...
static void veye_camera_module_reset(
	struct veye_camera_module *cam_mod)
{
	...
	cam_mod->state_before_suspend = VEYE_CAMERA_MODULE_POWER_OFF;
...
/* ======================================================================== */
//把一组配置设置下来,作为当前配置
static void veye_camera_module_set_active_config(
	struct veye_camera_module *cam_mod,
	struct veye_camera_module_config *new_config)
{
	pltfrm_camera_module_pr_debug(&cam_mod->sd, "\n");
	...
	//这里关于2A的其实可以注掉
		cam_mod->ctrl_updt &= VEYE_CAMERA_MODULE_CTRL_UPDT_AUTO_EXP |
			VEYE_CAMERA_MODULE_CTRL_UPDT_AUTO_GAIN |
			VEYE_CAMERA_MODULE_CTRL_UPDT_AUTO_WB;
		if (new_config->auto_exp_enabled !=
			cam_mod->exp_config.auto_exp) {
			cam_mod->ctrl_updt |=
				VEYE_CAMERA_MODULE_CTRL_UPDT_AUTO_EXP;
			cam_mod->exp_config.auto_exp =
...
/* ======================================================================== */
//根据分辨率之类信息，找到对应的config类型
static struct veye_camera_module_config *veye_camera_module_find_config(
	struct veye_camera_module *cam_mod,
...
/* ======================================================================== */
//把一组config配置，写到sensor中
static int veye_camera_module_write_config(
	struct veye_camera_module *cam_mod)
{
	int ret = 0;
	...
	ret = veye_camera_module_write_reglist(cam_mod,
		reg_table, reg_table_num_entries);
	if (IS_ERR_VALUE(ret))
		goto err;
...
//验证cameraid，绑定到系统上
static int veye_camera_module_attach(
	struct veye_camera_module *cam_mod)
{
	int ret = 0;
	...
	if (custom->check_camera_id) {
		//不用配置camera的power，永远on
		veye_camera_module_s_power(&cam_mod->sd, 1);
		ret = custom->check_camera_id(cam_mod);
		veye_camera_module_s_power(&cam_mod->sd, 0);
		if (ret != 0)
			goto err;
	}
...
/* ======================================================================== */
//数据格式是否支持?
int veye_camera_module_try_fmt(struct v4l2_subdev *sd,
	struct v4l2_mbus_framefmt *fmt)
{
	...
	if (IS_ERR_OR_NULL(veye_camera_module_find_config(cam_mod, fmt, NULL))) {
		pltfrm_camera_module_pr_debug(&cam_mod->sd,
	...
	struct v4l2_subdev_format *format)
{
	...
	if (cam_mod->frm_intrvl_valid) {
		veye_camera_module_set_active_config(cam_mod,
			veye_camera_module_find_config(cam_mod,
	...
	struct v4l2_subdev_frame_interval *interval)
{
	...
	unsigned long gcdiv;
	struct v4l2_subdev_frame_interval norm_interval;
	struct veye_camera_module_config *config;
	unsigned int vts;
	int ret = 0;
...
/* ======================================================================== */
//开始码流传输或者停止码流传输
int veye_camera_module_s_stream(struct v4l2_subdev *sd, int enable)
{
	int ret = 0;
	struct veye_camera_module *cam_mod =  to_veye_camera_module(sd);
	unsigned int vts;
	...
	if (enable) {
		...
		if (cam_mod->update_config) {
			ret = veye_camera_module_write_config(cam_mod);
			if (IS_ERR_VALUE(ret))
				goto err;
		}
		...
		cam_mod->state = VEYE_CAMERA_MODULE_STREAMING;
	} else {
		int pclk;
		int wait_ms;
		...
		ret = veye_camera_module_ioctl(sd,
...
/* ======================================================================== */
//控制power
int veye_camera_module_s_power(struct v4l2_subdev *sd, int on)
{
	int ret = 0;
	struct veye_camera_module *cam_mod =  to_veye_camera_module(sd);
	...
	if (on) {
		if (cam_mod->state == VEYE_CAMERA_MODULE_POWER_OFF) {
			...
			if (!IS_ERR_VALUE(ret)) {
				...
				cam_mod->state = VEYE_CAMERA_MODULE_HW_STANDBY;
			}
		}
		...
		if (cam_mod->state == VEYE_CAMERA_MODULE_HW_STANDBY) {
			...
			if (!IS_ERR_VALUE(ret)) {
				cam_mod->state = VEYE_CAMERA_MODULE_POWER_OFF;
				veye_camera_module_reset(cam_mod);
			}
		}
	}
...
/* ======================================================================== */
//获取是否支持各种控制
int veye_camera_module_g_ctrl(struct v4l2_subdev *sd,
	...
	struct v4l2_ext_controls *ctrls)
{
	...
	for (i = 0; i < ctrls->count; i++) {
		...
		switch (ctrl->id) {
		case V4L2_CID_GAIN:
			ctrl_updt = VEYE_CAMERA_MODULE_CTRL_UPDT_GAIN;
		...
		case V4L2_CID_FLASH_LED_MODE:
			...
			break;
		case V4L2_CID_EXPOSURE:
			ctrl_updt = VEYE_CAMERA_MODULE_CTRL_UPDT_EXP_TIME;
		...
		case RK_V4L2_CID_VTS:
			ctrl_updt = VEYE_CAMERA_MODULE_CTRL_UPDT_VTS_VALUE;
			cam_mod->exp_config.vts_value = ctrl->value;
			break;
		case V4L2_CID_WHITE_BALANCE_TEMPERATURE:
			ctrl_updt = VEYE_CAMERA_MODULE_CTRL_UPDT_WB_TEMPERATURE;
			...
			break;
		case V4L2_CID_AUTO_N_PRESET_WHITE_BALANCE:
			ctrl_updt = VEYE_CAMERA_MODULE_CTRL_UPDT_PRESET_WB;
			...
			break;
		case V4L2_CID_AUTOGAIN:
			ctrl_updt = VEYE_CAMERA_MODULE_CTRL_UPDT_AUTO_GAIN;
			...
			break;
		case V4L2_CID_EXPOSURE_AUTO:
			ctrl_updt = VEYE_CAMERA_MODULE_CTRL_UPDT_AUTO_EXP;
			...
			break;
		case V4L2_CID_AUTO_WHITE_BALANCE:
			ctrl_updt = VEYE_CAMERA_MODULE_CTRL_UPDT_AUTO_WB;
		...
		case V4L2_CID_FOCUS_ABSOLUTE:
			...
			ctrl_updt =
				VEYE_CAMERA_MODULE_CTRL_UPDT_FOCUS_ABSOLUTE;
	...
	if (ctrl_cnt &&
		(cam_mod->state == VEYE_CAMERA_MODULE_STREAMING ||
		cam_mod->state == VEYE_CAMERA_MODULE_SW_STANDBY)) {
		struct veye_camera_module_ext_ctrls tc_ctrls;

		tc_ctrls.ctrls =
		(struct veye_camera_module_ext_ctrl *)
		kmalloc(ctrl_cnt * sizeof(struct veye_camera_module_ext_ctrl),
			GFP_KERNEL);
	...
	struct v4l2_control *ctrl)
{
	...
	return veye_camera_module_s_ext_ctrls(sd, &ext_ctrls);
}
	...
	void *arg)
{
	...
	if (cmd == RK_VIDIOC_SENSOR_MODE_DATA) {
		struct veye_camera_module_timings veye_timings;
		struct isp_supplemental_sensor_mode_data *timings =
		(struct isp_supplemental_sensor_mode_data *)arg;

		if (cam_mod->custom.g_timings)
			ret = cam_mod->custom.g_timings(cam_mod, &veye_timings);
		else
			ret = -EPERM;
		...
		timings->sensor_output_width = veye_timings.sensor_output_width;
		timings->sensor_output_height = veye_timings.sensor_output_height;
		timings->crop_horizontal_start =
			veye_timings.crop_horizontal_start;
		timings->crop_vertical_start = veye_timings.crop_vertical_start;
		timings->crop_horizontal_end = veye_timings.crop_horizontal_end;
		timings->crop_vertical_end = veye_timings.crop_vertical_end;
		timings->line_length_pck = veye_timings.line_length_pck;
		timings->frame_length_lines = veye_timings.frame_length_lines;
		timings->vt_pix_clk_freq_hz = veye_timings.vt_pix_clk_freq_hz;
		timings->binning_factor_x = veye_timings.binning_factor_x;
		timings->binning_factor_y = veye_timings.binning_factor_y;
		timings->coarse_integration_time_max_margin =
			veye_timings.coarse_integration_time_max_margin;
		timings->coarse_integration_time_min =
			veye_timings.coarse_integration_time_min;
		timings->fine_integration_time_max_margin =
			veye_timings.fine_integration_time_max_margin;
		timings->fine_integration_time_min =
			veye_timings.fine_integration_time_min;
		...
		if (cam_mod->exp_config.exp_time)
			...
			timings->exp_time = veye_timings.exp_time;
		if (cam_mod->exp_config.gain)
			timings->gain = cam_mod->exp_config.gain;
		else
			timings->gain = veye_timings.gain;
		...
		*itf_cfg = config->itf_cfg;
		...
		ret = veye_camera_module_init(cam_mod, &cam_mod->custom);
		if (!IS_ERR_VALUE(ret)) {
			pltfrm_camera_module_ioctl(sd, cmd, arg);
			return veye_camera_module_attach(cam_mod);
		} else {
			pltfrm_camera_module_pr_err(&cam_mod->sd,
				"veye_camera_module_init    failed!\n");
			veye_camera_module_release(cam_mod);
			return ret;
		}
	}
	...
	struct veye_camera_module *cam_mod)
{
	...
	if (cam_mod->hflip)
		mode |= VEYE_MIRROR_BIT_MASK;
	else
		mode &= ~VEYE_MIRROR_BIT_MASK;

	if (cam_mod->vflip)
		mode |= VEYE_FLIP_BIT_MASK;
	else
		mode &= ~VEYE_FLIP_BIT_MASK;
	...
	struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	if (fie->index >= cam_mod->custom.num_configs)
		return -EINVAL;
	pltfrm_camera_module_pr_err(&cam_mod->sd, "confignum %d \n",cam_mod->custom.num_configs);
	pltfrm_camera_module_pr_err(&cam_mod->sd, "configs  %x \n",cam_mod->custom.configs);
	fie->code =
...
/* ======================================================================== */
static int veye_camera_module_i2c_write(struct v4l2_subdev *sd, u16 reg,
					u8 *values, u32 len)
{
...
/* ======================================================================== */
static int veye_camera_module_i2c_read(struct v4l2_subdev *sd, u16 reg,
					u32 *value, u32 len)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret = 0;
...
int veye_camera_module_write8_reg(
	...
	u8 val)
{
	return veye_camera_module_i2c_write(&cam_mod->sd, reg, &val, 1);
}
...
int veye_camera_module_write16_reg(
	...
	u16 val)
{
	return veye_camera_module_i2c_write(&cam_mod->sd, reg, (u8 *)&val, 2);
}
...
int veye_camera_module_write32_reg(
	...
	u32 val)
{
	return veye_camera_module_i2c_write(&cam_mod->sd, reg, (u8 *)&val, 4);
}
	...
	u16 reg)
{
	...
	veye_camera_module_i2c_read(&cam_mod->sd, reg, &val, 1);
	ret = val & 0xff;
	...
	u16 reg)
{
	...
	veye_camera_module_i2c_read(&cam_mod->sd, reg, &val, 2);
	ret = val & 0xffff;
	...
	u16 reg)
{
	...
	veye_camera_module_i2c_read(&cam_mod->sd, reg, &val, 4);
	...
	u32 *val)
{
	...
	if (cam_mod->state == VEYE_CAMERA_MODULE_STREAMING)
		return veye_camera_module_i2c_read(&cam_mod->sd,
			reg, val, 1);
	...
	int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		case VEYE_CAMERA_MODULE_REG_TYPE_DATA:
			switch (reglist[i].len) {
			case VEYE_CAMERA_MODULE_REG_VALUE_LEN_8BIT:
				veye_camera_module_write8_reg(cam_mod, reglist[i].reg, (u8)reglist[i].val);
				break;
			case VEYE_CAMERA_MODULE_REG_VALUE_LEN_16BIT:
				veye_camera_module_write16_reg(cam_mod, reglist[i].reg, (u16)reglist[i].val);
				break;
			case VEYE_CAMERA_MODULE_REG_VALUE_LEN_32BIT:
				veye_camera_module_write32_reg(cam_mod, reglist[i].reg, (u32)reglist[i].val);
				break;
			default:
...
int veye_camera_module_init(struct veye_camera_module *cam_mod,
	struct veye_camera_module_custom_config *custom)
{
	int ret = 0;
	int mode = 0;
	...
	if (mode != -1) {
		cam_mod->hflip = mode & VEYE_MIRROR_BIT_MASK ? true : false;
		cam_mod->vflip = mode & VEYE_FLIP_BIT_MASK ? true : false;
		cam_mod->flip_flg = true;
	}
	return 0;
err:
...
void veye_camera_module_release(struct veye_camera_module *cam_mod)
{
	pltfrm_camera_module_pr_debug(&cam_mod->sd, "\n");
