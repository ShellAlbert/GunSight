...
if [ $icam -eq 0 ]; then
	...
	vdev="/dev/video4"
	rkargs="device=${vdev} sensor-id=5"
fi
...
if [ -c ${vdev} ]; then
	echo "Start MIPI CSI Camera Preview [${vdev}] ..."
else
	echo "Error: ${vdev}: No such device"
	exit -1
fi
...
if [ "x$action" == "xphoto" ]; then
    GSTC="gst-launch-1.0 rkisp num-buffers=1 ${rkargs} io-mode=4 path-iqf=${iqf} \
        ! video/x-raw,format=NV12,${mode} \
